...
// Books.initEasySearch(['title','tags']);
 ...
 Books.deny({
 	update: function(userId, book, fieldNames,tags){
 		return (_.without(fieldNames,'url', 'title', 'pubDate','tags','stars','reviews','price','download_url','image_url','pages').length > 0);
 	}
 });
 ...
 Meteor.methods({
 	book: function(bookAttributes) {
 		...
 		var book = _.extend(_.pick(bookAttributes,'url','title','pubDate','tags','stars','reviews','price','download_url','image_url','pages'), {
 			userId: user._id,
